...
//! lane table as we go.
package io.github.kotlinmania.lalrpop_kotlin.lr1.laneTable.lane
...
class LaneTracer<L : Lookahead<L>>(
    ...
    fun startTrace(
        ...
        val visitedSet: Set<Pair<StateIndex, Lr0Item>> = set()
    ...
    private fun continueTrace(
        ...
        conflict: ConflictIndex,
        item: Lr0Item,
        visited: Set<Pair<StateIndex, Lr0Item>>,
    ) {
        // debug!("continue_trace:  state={:?}, index={:?}", state, item.index);
        if (!visited.add(Pair(state, item))) {
            return
        }
