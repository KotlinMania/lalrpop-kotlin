...
sealed class Comment<T> {
    ...
    override fun toString(): String = when (this) {
        is Goto -> " // on $token, goto $newState"
        is Error -> " // on $token, error"
        is Reduce -> " // on $token, reduce `$production`"
    }
}
...
private fun CodeGenerator<TableDriven>.writeTokenToSymbolFn() {
    ...
    for ((variantName, indices) in tokenToSymbolMapping) {
        ...
        val patterns: List<String> = indices.map { (_, pattern) ->
            val p = pattern as io.github.kotlinmania.lalrpop_kotlin.grammar.pattern.Pattern<TypeRepr>
            var nameIndex = 0
            val mapped = p.map {
