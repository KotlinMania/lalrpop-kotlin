...
sealed class Symbol : Comparable<Symbol> {
    ...
    // by `data class`-generated toString. Mirrors Rust's
    // `Display for Symbol`.
    ...
    override fun compareTo(other: Symbol): Int {
        // Mirror Rust's `#[derive(Ord)]` on the upstream enum:
        // sorts by variant declaration order first (Nonterminal < Terminal),
        // then by the variant's inner value. Toing-string-comparison gave
...
sealed class TypeRepr : Comparable<TypeRepr> {
    /**
     * Mirrors `impl Display for TypeRepr` AND `impl Debug for TypeRepr`
     * (the Debug impl defers to Display). Each subclass overrides
    ...
    override fun compareTo(other: TypeRepr): Int {
        // Mirror Rust's `#[derive(Ord)]` on `pub enum TypeRepr`: sort
        ...
        // upstream's recursive Ord derive for the cases LALRPOP
        // exercises.
