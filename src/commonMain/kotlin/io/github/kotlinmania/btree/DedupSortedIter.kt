...
// Kotlin stdlib has no Peekable adapter, so we inline a tiny one-element
...
// so `Pair<K, V>?` cleanly encodes the upstream `Option<(K, V)>`.
...
internal class DedupSortedIter<K, V, I : Iterator<Pair<K, V>>>(
    ...
    // Kotlin `Iterator` interface separates `hasNext` from `next`; the upstream
