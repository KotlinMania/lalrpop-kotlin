...
internal class LeafRange<BorrowType, K, V>(
    ...
    // and consumers of this port do not fork ranges. The two equivalent
     ...
     * }`.
     *
     * Caller-side discipline replaces the upstream BorrowType-restricted implementation block:
 ...
 * front slot is still in `Root` form. The caller-side equivalent of the upstream
        ...
        edge = when (val rk = edge.rightKv()) {
            ...
            is EdgeKvResult.Err -> when (val asc = rk.handle.intoNode().ascend()) {
                ...
                is AscendResult.Err -> {
                    ...
                    // ascend Err branch via type inference; here Node.kt
