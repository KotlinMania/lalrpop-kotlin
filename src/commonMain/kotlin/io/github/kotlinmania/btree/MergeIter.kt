 ...
 *
 * the upstream `implementation Clone` is omitted: cloning a Kotlin `Iterator<T>` is not generally
...
internal class MergeIterInner<T>(
     ...
     * contains a value, that value does not occur in the other source (or
     ...
     * are equal, positive when the first follows. This matches the upstream
    ...
    fun nexts(cmp: (T, T) -> Int): Pair<T?, T?> {
        ...
        // Clear the peeked slot now that we have moved its contents into a/bNext;
     ...
     *
     * Rust constrains this method with `I: ExactSizeIterator`; Kotlin
    ...
    /**
     * Mirrors the upstream `implementation Debug for MergeIterInner`. The peeked slot is rendered
