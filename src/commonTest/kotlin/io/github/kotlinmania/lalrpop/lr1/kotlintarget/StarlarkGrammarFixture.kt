...
/**
 * The Starlark grammar source from `starlark-kotlin/tmp/starlark_syntax/src/syntax/grammar.lalrpop`.
 *
 * Copied verbatim from upstream so we can run the lalrpop-kotlin front end on it and cross-check
 * what it produces against starlark-kotlin's hand-translated `Grammar.kt` /
 * `GrammarReducers.kt` / `GrammarSymbol.kt`.
 *
 * Source license: Apache 2.0 (see header comment inside the string for upstream attribution).
 */
internal const val STARLARK_GRAMMAR_LALRPOP: String = """// Copyright 2018 The Starlark in Rust Authors.
